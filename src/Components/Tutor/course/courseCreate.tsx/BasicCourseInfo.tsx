...
const BasicCourseInfo: React.FC<BasicCourseInfoProps> = ({ onSubmit }) => {
  ...
  const handleThumbnailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (file && file.type.startsWith("image/") ) {
        ...
        setErrors((prev) => ({ ...prev, thumbnail: "Please upload a valid image file (max 5MB)." }));
      }
    }
  };
  ...
  const handleVideoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (file && file.type.startsWith("video/")) {
        ...
        setErrors((prev) => ({ ...prev, video: "Please upload a valid video file (max 50MB)." }));
      }
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!trimmedDescription) formErrors.description = "Description is required.";
    if (trimmedDescription.length > maxLengthDescription) formErrors.description = `Description cannot exceed ${maxLengthDescription} characters.`;
    if (!category) formErrors.category = "Category is required.";
    if (!trimmedPrice || isNaN(Number(trimmedPrice)) || Number(trimmedPrice) <= 0) formErrors.price = "Price must be a positive number.";
