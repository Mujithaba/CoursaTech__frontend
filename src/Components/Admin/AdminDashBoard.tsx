    ...
    <div className="p-8 bg-teal-900 text-white min-h-screen rounded-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card
          ...
          amount={`$${dashboardData.totalEarnings.toFixed(2)}`}
          icon={<DollarSign />}
        />
        <Card
          title="Total Students"
          amount={dashboardData.totalStudents.toString()}
          icon={<Users />}
        />
        <Card
          title="Active Courses"
          amount={dashboardData.activeCourses.toString()}
          icon={<Book />}
        />
      </div>
      ...
      {enrollments.map((item, idx) => (
        <li key={idx} className="flex justify-between mb-2">
          ...
          <span className="text-green-500">
            {item.rating.toFixed(1)} <span className="animate-pulse">⭐</span>
          </span>
        </li>
      ))}
    </ul>
